...
async def encc(e):
    try:
        ...
        cmd = f'ffmpeg -i  "{dl}" -map 0 -c:v h265x10bit -metadata title=HEVCEncodes -pix_fmt yuv420p -preset fast -crf 27 -b:v 2M -profile:a  aac_he_v2 -c:a aac -ac 2 -vbr 2 -ab 40k -c:s copy "{out}" -y'
        process = await asyncio.create_subprocess_shell(
